...
# =====================
file_base = 'medium'
osm_file = f'../{file_base}.osm'
output_file = f'{file_base}_full.png'
...
# 特殊点 A,B,C... & 圆
# =====================
special_points_lonlat = [
    (116.9998, 40.4977), (117.0004, 40.4979),
    (116.9998, 40.497702),(116.999815, 40.497702),(117.000158, 40.497704),
    (117.000524, 40.497709),(117.000524, 40.4979)
]
special_labels = ['A', 'B', 'C', 'D', 'E', 'F', 'G']
circle_radius_m = 82.5  # 半径
...
# --- 特殊点 A,B,C...
for (lon, lat), label in zip(special_points_lonlat, special_labels):
    px, py = latlon_to_xy(lat, lon, lat0, lon0)
    ax.scatter(px, py, s=60, c='red', edgecolor='black', zorder=60)
    ax.text(px + 1.0, py + 1.0, label, fontsize=9, fontweight='bold', color='black', zorder=61)

# --- 绘制以 A 为圆心的圆
if special_points_lonlat:
    lonA, latA = special_points_lonlat[0]
    cx, cy = latlon_to_xy(latA, lonA, lat0, lon0)
    circ = Circle((cx, cy), circle_radius_m, fill=False, edgecolor='red', linestyle='--', linewidth=1.5, zorder=100, clip_on=False)
    ax.add_patch(circ)
    ax.text(cx + circle_radius_m, cy, f"R={circle_radius_m}m", fontsize=8, color='red')
...
ax.set_title("OSM Map with Units + Special Points + Circle")
