...
A[接收重规划请求<br/>PoseArray] --> B{路径是否为空}
B -- 是 --> Z[返回原路径]
B -- 否 --> C[读取最新点云数据<br/>LiDAR + Camera]

C --> D[坐标变换到 map 坐标系]
D --> E[高度过滤]

E --> F[生成局部栅格地图<br/>local costmap]
F --> G[障碍物膨胀<br/>距离变换]
...
- [delivery_executor_action_server.yaml](src/delivery_executor/delivery_executor/delivery_executor_action_server.py)
